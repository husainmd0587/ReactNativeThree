...
function buildLeft(profile, partingZ) {
  ...
  for (const p of profile) {
    if (p.z <= partingZ) clipped.push(p)
    else break
  }
...
// build right part from partingZ + partingWidth onwards
function buildRight(profile, partingZ, partingWidth) {
  const cutEnd = partingZ + partingWidth
  const clipped = []
  for (const p of profile) {
    if (p.z >= cutEnd) clipped.push(p)
  }
  const rAtCut = radiusAtZ(profile, cutEnd)
  clipped.unshift({ z: cutEnd, r: rAtCut })
...
export default function Grooving() {
  return (
